...
def dump_seq_ack(seq, ack, wdir):
	if seq != -1:
		f = open(wdir + 'seq.txt', 'w')
		f.write(str(seq))
		f.close()

	if ack != -1:
		f = open(wdir + 'ack.txt', 'w')
		f.write(str(ack))
		f.close()

def get_time_ms():
	milli_sec = int(round(time.time() * 1000))
	return milli_sec - 1524000000000

def raise_imp_exception(s, cn):
	if cn == 1:
		print('*********************************************')
		print(s)
		print('*********************************************')
...
# def ack_queue_monitor(ack_qu):
# 	while True:
# 		try:
# 			curr_time = time.time()

# 			# print('before starting reading qu...')

# 			if ack_qu.qsize() > 0:

# 				print(curr_time, ack_qu.qsize())
# 				# # print('starting reading qu...')
# 				# res = ack_qu.queue[0]
# 				# # print('read qu:', res)
...
# 				# print(curr_time, pkt_time, curr_time - pkt_time)

# 				# if curr_time - pkt_time >= time_to_wait:

# 				# 	# self.send_ack_raw_single(seq_to_ack, wait_flag)
# 				# 	ack_qu.get()

# 				# else:
# 				# 	time.sleep(100 / 1000)
# 			else:
# 				print('qsize:', ack_qu.qsize())
# 				time.sleep(0.1)

# 		except Exception,e:
# 			print('exception_in_ack_process')
# 			pass
...
class TLSSocket(object):
	def __init__(self, input_tcp_obj=None, sock=None, client=None, tls_ctx=None):
		...
		self.ACK = -1
		self.MSS = -1
		self.MSS_AFTER_OPTS = 52
		# self.MSS_AFTER_OPTS = 1460
		self.CONF_DIR = '/home/usama/github/inspector-gadget-py/cc/v3/conf/'
		self.RESULTS_DIR = ''
		self.LOG_FILE_NAME = ''
		self.TLS_HS_DONE = False
		self.RTT_PAST = 0
		self.NO_DATA_COUNTER_THRESHOLD = 100
		self.LAST_CWND = 0
		self.TCP_TIMESTAMP = 0
		self.IP_ID = random.randint(100, 3000)
		self.RTTS_WITH_NO_DATA_CONSECUTIVE = 0

		# ACK thread related data structures
		self.ACK_PROCESS_CREATED = False
		# self.DATA_TO_ACK_QUEUE = Queue.Queue()
		self.DATA_TO_ACK_QUEUE = multiprocessing.Queue()
		self.SECONDARY_QUEUE = multiprocessing.Queue()
		self.ACK_PROCESS = 0

		# loss emulation related data structures
		self.SHOULD_EMULATE_LOSS = True
		self.LOSS_EMULATION_STARTED = False
		self.LOSS_EMULATION_RTT_THRESHOLD = 0
		self.LOSS_EMULATION_CWND_THRESHOLD = 0
		self.SEQ_AT_LOSS = 0
		self.CWND_AT_LOSS = 0
		self.RTT_AT_LOSS = 0
		self.SEQ_CHECKER = 0
		self.DUP_ACKS_SEND = 0
		self.LOSS_RECOVERY_DONE = False
		self.START_DATA_ACKED = 0
		self.END_DATA_ACKED = 0

		# emualte RTT increase
		self.RTT_INC = False
		self.RTT_INC_VALUE = 0

		# BBR relared data structures
		self.BBR_TEST = False
		self.BBR_INPUT_CWND = 0
		self.BBR_EST_THPUT = 0
		self.BBR_BASE_RTT = 0
		...
		try:
			...
			for row in cr:
				...
				self.SEQ = int(row[5])
				self.ACK = int(row[6])
				self.LOSS_EMULATION_RTT_THRESHOLD = int(row[7])
				self.LOSS_EMULATION_CWND_THRESHOLD = int(row[8])
				self.TCP_TIMESTAMP = int(row[9])
				self.SRC_IP = row[10]
				self.RTT_INC_VALUE = float(row[11])
				self.CONF_DIR = row[12]
				self.RESULTS_DIR = row[13]
				self.LOG_FILE_NAME = row[14]
				self.BBR_INPUT_CWND = int(row[15])

		except Exception, e:
			print(e, 'conn-info.csv dont exist')
			sys.exit()

		self.LOG_FILE = open(self.RESULTS_DIR + self.LOG_FILE_NAME + '-log.txt', 'w')

		print('conn-info:', self.SRC_IP, self.DST_IP, self.SRC_PORT, self.DST_PORT, self.EMULATED_RTT, self.MSS, self.LOSS_EMULATION_RTT_THRESHOLD, self.LOSS_EMULATION_CWND_THRESHOLD, self.RTT_INC_VALUE)
		print('SEQ/ACK:', self.SEQ, self.ACK, self.TCP_TIMESTAMP)

		# self.SOCKET_FOR_ACKS = socket.socket(socket.AF_INET, socket.SOCK_RAW, socket.IPPROTO_TCP)
		# self.SOCKET_FOR_ACKS.setsockopt(socket.SOL_IP, socket.IP_HDRINCL, 1)

		# try:
		# 	self.SOCKET_FOR_ACKS = socket.socket(socket.AF_INET, socket.SOCK_RAW, socket.IPPROTO_RAW)
		# except socket.error , msg:
		# 	raise Exception('ack_socket_not_open')
		
		# self.SOCKET_FOR_ACKS = socket.socket(socket.AF_INET, socket.SOCK_RAW, socket.IPPROTO_RAW)
		# self.SOCKET_FOR_ACKS.setsockopt(socket.SOL_SOCKET, socket.SO_SNDBUF, 12582912)
		# print('SO_SNDBUF:', self.SOCKET_FOR_ACKS.getsockopt(socket.SOL_SOCKET, socket.SO_SNDBUF))

		# self.ACK_TEMPLATE = IP(dst=self.DST_IP) / TCP(sport=self.SRC_PORT, dport=self.DST_PORT, seq=0, ack=0, window=65535, flags="A")
		# self.ACK_TEMPLATE_HEX = binascii.hexlify(str(self.ACK_TEMPLATE))

		if self.RTT_INC_VALUE != 0:
			self.RTT_INC = True

		if self.LOSS_EMULATION_RTT_THRESHOLD == 0 or self.LOSS_EMULATION_CWND_THRESHOLD == 0:
			self.SHOULD_EMULATE_LOSS = False

		if self.BBR_INPUT_CWND != 0:
			self.BBR_TEST = True
			# self.SHOULD_EMULATE_LOSS = False
			# self.RTT_INC = False
	...
	def send_acks_raw(self, count):

		sleep_time = 0
		if count == 4:
			sleep_time = 0.005
		
		if len(self.PACKETS_TO_ACK) > 0 and count > 0:

			try:
				self.START_DATA_ACKED = self.PACKETS_TO_ACK[0][0] - self.MSS_AFTER_OPTS
				self.END_DATA_ACKED = self.PACKETS_TO_ACK[len(self.PACKETS_TO_ACK) - 1][0]

				s = socket.socket(socket.AF_INET, socket.SOCK_RAW, socket.IPPROTO_RAW)
				s.setsockopt(socket.SOL_SOCKET, socket.SO_SNDBUF, 12582912)
				print(s.getsockopt(socket.SOL_SOCKET, socket.SO_SNDBUF))

				# s = self.SOCKET_FOR_ACKS

				acks_sent = 0
				for i in self.PACKETS_TO_ACK:
					packet = ''
					# source_ip = '192.168.0.14'
					# source_ip = '10.38.25.174'
					...
					self.IP_ID += random.randint(1, 555)
					if self.IP_ID > 65535:
						self.IP_ID = random.randint(10, 5550)
					ip_id = self.IP_ID				
					...
					# else:
					# 	if acks_sent == len(self.PACKETS_TO_ACK) - 1:
					# 		time.sleep(0.001)
			...
			print('No pkts to ack.')


	def send_new(self, pkt_to_be_sent):
		# custom_options = [("NOP",None), ("NOP",None), ('Timestamp', (get_time_ms(), self.TCP_TIMESTAMP))]
		custom_options = []
		# newpkt = IP(dst=self.DST_IP) / TCP(sport=self.SRC_PORT, dport=self.DST_PORT, seq=self.SEQ, ack=self.ACK, window=65535, flags="PA", options=custom_options) / pkt_to_be_sent
		newpkt = IP(dst=self.DST_IP) / TCP(sport=self.SRC_PORT, dport=self.DST_PORT, seq=self.SEQ, ack=self.ACK, window=65535, flags="PA") / pkt_to_be_sent
		send(newpkt)
		print('send_new:', self.SEQ, self.ACK, len(pkt_to_be_sent))
		print '.'
		self.SEQ += len(pkt_to_be_sent)

	def send_acks(self, count):
		if len(self.PACKETS_TO_ACK) > 0 and count > 0:

			sockt = socket.socket(socket.AF_INET, socket.SOCK_RAW, socket.IPPROTO_TCP)
			sockt.setsockopt(socket.SOL_IP, socket.IP_HDRINCL, 1)

			custom_options = [("NOP",None), ("NOP",None), ('Timestamp', (get_time_ms(), self.TCP_TIMESTAMP))]
			new_pkt = IP(dst=self.DST_IP) / TCP(sport=self.SRC_PORT, dport=self.DST_PORT, seq=self.SEQ, ack=0, window=65535, flags="A", options=custom_options)
			temp = bytes(new_pkt)

			for i in self.PACKETS_TO_ACK:
				# new_pkt[TCP].ack = i[0]
				sockt.sendto(bytes(new_pkt), (self.DST_IP, self.DST_PORT))
				# sockt.sendto(temp, (self.DST_IP, self.DST_PORT))
	...
	def send_last_ack(self):
		if len(self.PACKETS_TO_ACK) > 0:
			new_ack = self.PACKETS_TO_ACK[len(self.PACKETS_TO_ACK) - 1][0]
			new_pkt = IP(dst=self.DST_IP) / TCP(sport=self.SRC_PORT, dport=self.DST_PORT, seq=self.SEQ, ack=new_ack, window=65535, flags="A")
			
			for i in range(0,2):
				self.SOCKET_FOR_ACKS.sendto(bytes(new_pkt), (self.DST_IP, self.DST_PORT))
	...
	def send_last_ack_by_scapy(self, count):
		if len(self.PACKETS_TO_ACK) > 0 and count > 0:
			new_ack = self.PACKETS_TO_ACK[len(self.PACKETS_TO_ACK) - 1][0]
			# new_pkt = IP(dst=self.DST_IP) / TCP(sport=self.SRC_PORT, dport=self.DST_PORT, seq=self.SEQ, ack=new_ack, window=65535, flags="A", options=[("WScale", random.randint(4, 8))])
			
			# custom_options = [("NOP",None), ("NOP",None), ("NOP",None), ("NOP",None), ("NOP",None), ("NOP",None), ("SAckOK", ""), ("NOP",None), ("WScale",8), ("EOL",None)]
			custom_options = [("NOP",None), ("NOP",None), ('Timestamp', (get_time_ms(), self.TCP_TIMESTAMP))]
			new_pkt = IP(dst=self.DST_IP) / TCP(sport=self.SRC_PORT, dport=self.DST_PORT, seq=self.SEQ, ack=new_ack, window=65535, flags="A", options=custom_options)
			pkt_list = []
	...
	def send_acks_by_scapy(self, count):
		if len(self.PACKETS_TO_ACK) > 0 and count > 0:
			...
			for i in self.PACKETS_TO_ACK:
				new_pkt = IP(dst=self.DST_IP) / TCP(sport=self.SRC_PORT, dport=self.DST_PORT, seq=self.SEQ, ack=i[0], window=65535, flags="A", options=custom_options)
				pkt_list.append(new_pkt)
	...
	def reorder_pkts(self, pkts_received):
		temp = sorted(pkts_received, key=operator.itemgetter(0))
		return temp

	def detect_loss_and_duplicates(self, pkts_received_in_order):
		loss_flag = 0
		dup_flag = 0
		prev_seq = 0
		prev_len = 0
		...
		for i in pkts_received_in_order:
			if prev_seq == 0:
				prev_seq = i[0]
				prev_len = i[2]
				resp.append(i[3])
				timestamp_arr.append(i[4])
	
			else:
				if i[0] == prev_seq: # duplicate packet
					dup_flag = 1
					...
					loss_flag = 1
					return [loss_flag, dup_flag, resp, timestamp_arr]

		return [loss_flag, dup_flag, resp, timestamp_arr]



	def send_ack_raw_single(self, s, seq_to_ack, wait_flag, src_ip, src_port, dst_ip, dst_port, seq_to_seq, ipid):
		sleep_time = 0
		if wait_flag == 1:
			sleep_time = 0.005
	...
	def ack_queue_monitor(self, ack_qu, secondary_qu, src_ip, src_port, dst_ip, dst_port):
		socket_for_acks = socket.socket(socket.AF_INET, socket.SOCK_RAW, socket.IPPROTO_RAW)
		socket_for_acks.setsockopt(socket.SOL_SOCKET, socket.SO_SNDBUF, 12582912)
		print('SO_SNDBUF:', socket_for_acks.getsockopt(socket.SOL_SOCKET, socket.SO_SNDBUF))

		ipid = random.randint(1, 5555)

		fw = open(self.RESULTS_DIR + 'pid.csv', 'w')
		cw = csv.writer(fw)
		pr_id = os.getpid()
		cw.writerow([pr_id])
		fw.close()
		...
		while True:
			try:
				...
				if ack_qu.qsize() > 0 or secondary_qu.qsize() > 0:
					# print('qsize:', ack_qu.qsize())
					
					res = []
					if secondary_qu.qsize() > 0:
						res = secondary_qu.get()
					elif ack_qu.qsize() > 0:
						res = ack_qu.get()
					...
					wait_flag = res[3]
					seq_to_seq = res[4]
					# print(curr_time, pkt_time, curr_time - pkt_time, time_to_wait, seq_to_ack)

					if curr_time - pkt_time >= time_to_wait:
						ipid += random.randint(1, 15)
						if ipid > 65535:
							ipid = random.randint(10, 5555)

						self.send_ack_raw_single(socket_for_acks, seq_to_ack, wait_flag, src_ip, src_port, dst_ip, dst_port, seq_to_seq, ipid)
						# print('ack_sent:', seq_to_ack)
					else:
						secondary_qu.put(res)
						# time.sleep(time_to_wait - (curr_time - pkt_time))
	...
	def recvall(self, size=8192*2, timeout=2):

		if self.ACK_PROCESS_CREATED == False:
			self.ACK_PROCESS_CREATED = True
			self.ACK_PROCESS = multiprocessing.Process(target=self.ack_queue_monitor, args=(self.DATA_TO_ACK_QUEUE, self.SECONDARY_QUEUE, self.SRC_IP, self.SRC_PORT, self.DST_IP, self.DST_PORT, ))
			self.ACK_PROCESS.start()
		...
		cn = 0
		prev_timeout = self._s.gettimeout()
		self._s.settimeout(self.EMULATED_RTT / 10)
		# self._s.settimeout(self.EMULATED_RTT)
		# self._s.settimeout(2)
		no_pkt_received_yet = True
		first_pkt_receive_time = 0
		...
		while counter < 1000:
				...
				try:
					...
					if temp.haslayer(TCP):
						...
						if temp[TCP].dport == self.tcp_obj.src_port and (tcp_flag == 'PA' or tcp_flag == 'A'):
							...
							# print(2)
							if pkt_len > self.MSS_AFTER_OPTS:
								self.MSS_AFTER_OPTS = pkt_len

							if pkt_len > 0:
								if no_pkt_received_yet == True:
									no_pkt_received_yet = False
									first_pkt_receive_time = time.time()

								curr_cwnd += 1
								# print(3)

								if self.SHOULD_EMULATE_LOSS == True:
									if self.RTT_PAST == self.LOSS_EMULATION_RTT_THRESHOLD and self.LOSS_EMULATION_STARTED == False:
										print('\n\n#########   LOSS_EMULATION_STARTED   #########\n')
										self.LOSS_EMULATION_STARTED = True
										self.RTT_AT_LOSS = self.RTT_PAST
										self.SEQ_CHECKER = temp[TCP].seq
										self.SEQ_AT_LOSS = temp[TCP].seq
										# self.PACKETS_TO_ACK.append([self.SEQ_AT_LOSS, self.ACK, temp.time])

								# self.LOG_FILE.write('recv pkt len: %d, %d, %d, %d, %d, %d, %d\n'%(len(data[pkt_offset:len(data)]), pkt_offset, temp[TCP].seq, temp[TCP].seq + pkt_len, temp[TCP].ack, len(data), self.LAST_CWND))
								# print(4)
								if self.LOSS_EMULATION_STARTED == False:
									pkts_received.append([temp[TCP].seq, temp[TCP].ack, pkt_len, data[pkt_offset:len(data)], temp.time])

									# self.PACKETS_TO_ACK.append([temp[TCP].seq + pkt_len, self.ACK, temp.time])
									self.DATA_TO_ACK_QUEUE.put([temp[TCP].seq + pkt_len, temp.time, self.EMULATED_RTT, 0, self.SEQ])

									if temp[TCP].seq + pkt_len > self.ACK:
										self.ACK += pkt_len

									# for i in temp[TCP].options:
									# 	if i[0] == 'Timestamp':
									# 		self.TCP_TIMESTAMP = i[1][0]

								else:
									if self.RTT_PAST == self.RTT_AT_LOSS:
										self.DUP_ACKS_SEND += 1
										if self.DUP_ACKS_SEND <= 3:
											# self.PACKETS_TO_ACK.append([self.SEQ_AT_LOSS, self.ACK, temp.time])
											self.DATA_TO_ACK_QUEUE.put([self.SEQ_AT_LOSS, temp.time, self.EMULATED_RTT, 1, self.SEQ])

									elif self.RTT_PAST > self.RTT_AT_LOSS:
										if self.LOSS_RECOVERY_DONE == False:
											if temp[TCP].seq == self.SEQ_CHECKER:
												self.LOSS_RECOVERY_DONE = True
												# self.PACKETS_TO_ACK.append([temp[TCP].seq + pkt_len, self.ACK, temp.time])
												self.DATA_TO_ACK_QUEUE.put([temp[TCP].seq + pkt_len, temp.time, self.EMULATED_RTT, 0, self.SEQ])
											else:
												# self.PACKETS_TO_ACK.append([self.SEQ_CHECKER, self.ACK, temp.time])
												self.DATA_TO_ACK_QUEUE.put([self.SEQ_CHECKER, temp.time, self.EMULATED_RTT, 0, self.SEQ])
					...
					if no_pkt_received_yet == True:
						if (time.time() - start_time) >= self.EMULATED_RTT:
							break
					else:
						if (time.time() - first_pkt_receive_time) >= self.EMULATED_RTT:
							break
			...
			if no_pkt_received_yet == False and (end_time - first_pkt_receive_time) < self.EMULATED_RTT:
				time.sleep(self.EMULATED_RTT - (end_time - first_pkt_receive_time))

			# print(6)
			
			# # Send ACKs
			# if self.LOSS_EMULATION_STARTED == True and self.RTT_PAST == self.RTT_AT_LOSS:
			# 	# self.send_last_ack_by_scapy(3)
			# 	# self.send_acks_by_scapy(3)
			# 	# self.send_acks(3)
			# 	self.send_acks_raw(4)

			# elif self.LOSS_EMULATION_STARTED == True:
			# 	# self.send_last_ack_by_scapy(curr_cwnd)
			# 	# self.send_acks_by_scapy(3)
			# 	# self.send_acks(3)
			# 	self.send_acks_raw(3)

			# else:
			# 	# self.send_last_ack_by_scapy(2)
			# 	# self.send_acks_by_scapy(3)
			# 	# self.send_acks(3)
			# 	self.send_acks_raw(3)

			dump_seq_ack(self.SEQ, self.ACK, self.CONF_DIR)

			pkts_received_in_order = self.reorder_pkts(pkts_received)
			if len(pkts_received_in_order) > 0:
				self.START_DATA_ACKED = pkts_received_in_order[0][0]
				self.END_DATA_ACKED = pkts_received_in_order[len(pkts_received_in_order)-1][0]
			else:
				self.START_DATA_ACKED = -1
				self.END_DATA_ACKED = -1


			ret = self.detect_loss_and_duplicates(pkts_received_in_order)
			...
			if self.BBR_TEST == True:
				if self.BBR_EST_THPUT == 0:
					self.BBR_EST_THPUT = self.BBR_INPUT_CWND / self.EMULATED_RTT
					self.BBR_BASE_RTT = self.EMULATED_RTT

				curr_thput = curr_cwnd / self.BBR_BASE_RTT

				if curr_thput > self.BBR_EST_THPUT:
					self.EMULATED_RTT = curr_cwnd / self.BBR_EST_THPUT
				else:
					self.EMULATED_RTT = self.BBR_BASE_RTT


			
			if loss_flag == 1 and self.TLS_HS_DONE == False:
				# raise Exception('packet_loss_in_recv_during_TLS_HS')
				raise_imp_exception('packet_loss_in_recv_during_TLS_HS', 2)
				# if curr_cwnd > 5:
				# 	total_data_len = self.END_DATA_ACKED - self.START_DATA_ACKED
				# 	self.LAST_CWND = math.ceil(total_data_len / self.MSS_AFTER_OPTS)
				# 	print(curr_cwnd, self.LAST_CWND)
				# else:
				# 	self.LAST_CWND = curr_cwnd
				self.LAST_CWND = curr_cwnd
				...
				raise_imp_exception('loss_experienced', 1)
				# if curr_cwnd > 5:
				# 	total_data_len = self.END_DATA_ACKED - self.START_DATA_ACKED
				# 	self.LAST_CWND = math.ceil(total_data_len / self.MSS_AFTER_OPTS)
				# 	print(curr_cwnd, self.LAST_CWND)
			...
			print('Time elapsed: %d | RTT_PAST: %d | CWND: %d | RTT: %f'%(end_time-start_time, self.RTT_PAST, self.LAST_CWND, self.EMULATED_RTT))
			print('loss_flag:', loss_flag, '| dup_flag:', dup_flag, '| counter:', counter, '| TLS_HS_DONE:', self.TLS_HS_DONE)
			print('queue_len:', self. DATA_TO_ACK_QUEUE.qsize())
			
			cw = csv.writer(open(self.RESULTS_DIR + self.LOG_FILE_NAME + '-trace.csv', 'a'))
			if self.LOSS_EMULATION_STARTED == True and self.RTT_PAST == self.RTT_AT_LOSS:
				cw.writerow([-1, -1, -1, -1, -1, -1, time.time(), []])
			cw.writerow([self.LAST_CWND, curr_cwnd, self.EMULATED_RTT, self.RTT_PAST, self.START_DATA_ACKED, self.END_DATA_ACKED, time.time(), timestamp_arr])

			# RTT calculation
			self.RTT_PAST += 1
			if self.LOSS_EMULATION_STARTED == True and self.RTT_INC == True:
				self.EMULATED_RTT += self.RTT_INC_VALUE / 1000

			# quit of no data is received in some time
			if self.LAST_CWND == 0:
				self.RTTS_WITH_NO_DATA_CONSECUTIVE += 1
			else:
				self.RTTS_WITH_NO_DATA_CONSECUTIVE = 0

			if self.RTTS_WITH_NO_DATA_CONSECUTIVE >= 10:
				self.ACK_PROCESS.terminate()
				raise Exception('no_received_for_sometime')

			if self.RTT_PAST >= 90:
				self.ACK_PROCESS.terminate()
				raise Exception('enough_rtts_past')


			last_pkt_size = 0
			for i in resp_current:
				resp.append(i)
				cn += len(i)
				last_pkt_size = len(i)

			print(last_pkt_size, cn)
			if last_pkt_size == 0:
				no_data_counter += 1
			else:
				no_data_counter = 0

			if no_data_counter > self.NO_DATA_COUNTER_THRESHOLD:
				self.ACK_PROCESS.terminate()
				raise Exception('no_data_recieved_in_lat_%s_rtts'%(str(self.NO_DATA_COUNTER_THRESHOLD)))

			# if self.TLS_HS_DONE == False:
			if True:
				try:
					if last_pkt_size != 0 and last_pkt_size < self.MSS_AFTER_OPTS and cn > 0 and self.LOSS_EMULATION_STARTED == False:
						records = TLS("".join(resp), ctx=self.tls_ctx, _origin=self._get_pkt_origin('in'))
						print('Resp forwarded.', cn)
						self._s.settimeout(prev_timeout)
						return records

					else:
						print('Resp held. Waiting for more data.', cn, self.LOSS_EMULATION_STARTED)
				except:
					raise_imp_exception('EXCEPTION: Resp held. Waiting for more data. ' + str(cn), 2)
					pass
	...
	def do_handshake(self, version, ciphers, extensions=[]):
		ret = tls_do_handshake(self, version, ciphers, extensions)
		self.TLS_HS_DONE = True
		return ret
