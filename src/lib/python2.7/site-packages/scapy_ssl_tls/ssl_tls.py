...
class TLSSocket(object):
	def __init__(self, input_tcp_obj=None, sock=None, client=None, tls_ctx=None,dir=None):
		...
		self.INCREASE_RTT = True
		self.BBR_INPUT_CWND = 0
		self.BBR_EST_THPUT = 0
		self.BBR_BASE_RTT = 0

		# dealing with error like missing segment
		self.LAST_RTT_SEQ = 1
		self.TEST_FAIL = False
		self.LOSS_EMULATION_HAPPENED = False
		self.MAX_CWND = 0

		self.FRTO = False
		self.FRTO_NEW = 0
		self.FRTO_handled = False
		self.RTT_AT_RETRANS = -1
		...
		try:
			...
			for row in cr:
				...
				self.SEQ = int(row[5])
				self.ACK = int(row[6])
				self.LOSS_EMULATION_CWND_THRESHOLD = int(row[7])
				self.TCP_TIMESTAMP = int(row[8])
				self.SRC_IP = row[9]
				self.RTT_INC_VALUE = 0.2

		except Exception, e:
			print(e, 'conn-info.csv dont exist')
			print(self.CONF_DIR + 'conn-info.csv')
			sys.exit()

		if self.RTT_INC_VALUE != 0:
			self.RTT_INC = True

		if self.LOSS_EMULATION_RTT_THRESHOLD == 0 and self.LOSS_EMULATION_CWND_THRESHOLD == 0:
	...
	def ack_queue_monitor(self, ack_qu, secondary_qu, ACK_QUEUE,src_ip, src_port, dst_ip, dst_port):
		...
		while True:
			try:
				...
				if (ack_qu.qsize() > 0 or secondary_qu.qsize() > 0) and ACK_QUEUE.qsize() > 0:
					# print('qsize:', ack_qu.qsize())
	...
	def recvall(self, size=200, timeout=2):

		if self.ACK_PROCESS_CREATED == False:
			self.ACK_PROCESS_CREATED = True
			self.ACK_PROCESS = multiprocessing.Process(target=self.ack_queue_monitor, args=(self.DATA_TO_ACK_QUEUE, self.SECONDARY_QUEUE, self.ACK_QUEUE, self.SRC_IP, self.SRC_PORT, self.DST_IP, self.DST_PORT, ))
		...
		seq_before_recover = 0
		expect_seq = 0
		wait_time = 0
		no_data_received_last_rtt =True
		loss_cwnd = 0

		while counter < 1000 and self.TEST_FAIL == False:
			pkts_received = []
			curr_cwnd = 0
			loss_cwnd = 0
			no_pkt_received_yet = True
			...
			while True:
				try:
					...
					temp = Ether(data)
					deal_begin_time = time.time()
					temp.time -= deal_begin_time - deal_end_time + wait_time
					if hasattr(temp, 'haslayer')== True and  temp.haslayer(TCP):
						if temp[TCP].dport == self.tcp_obj.src_port:
							...
							if pkt_len > 0:
								#temp.time -= deal_end_time - deal_begin_time + wait_time
								if no_pkt_received_yet == True:
									no_pkt_received_yet = False
									first_pkt_receive_time = time.time()
									last_packet_receive_time = first_pkt_receive_time
									if no_data_received_last_rtt==True and counter<3:
										expect_seq = temp.seq

								last_packet_receive_time = temp.time

								#=================================================
								#update estimate congestion window
								#=================================================
								if self.LOSS_EMULATION_STARTED == True and self.LOSS_RECOVERY_DONE == False and temp.seq > self.LOSS_SEQ_CHECKER:
									seq_before_recover = temp.seq
									curr_cwnd += 1
									continue

								if (temp.seq == expect_seq):
									curr_cwnd += 1
									expect_seq = temp.seq + pkt_len
								else:
									if temp.seq > expect_seq:
										if self.TLS_HS_DONE == False:
											self.TEST_FAIL = True
											print('packet_loss_in_recv_during_TLS_HS')
											raise_imp_exception('packet_loss_in_recv_during_TLS_HS', 2)
										else:
											if self.LOSS_RECOVERY_DONE == True and temp.seq <=(1+self.FRTO_NEW+1)*pkt_len+seq_before_recover and self.FRTO_handled == False:
												if temp.seq == seq_before_recover + (1+ self.FRTO_NEW) * pkt_len:
													print("FRTO is used, the new packet is ", temp.seq)
													if self.FRTO == False:
														self.FRTO = True
													self.FRTO_NEW += 1
												continue
												...
												self.TEST_FAIL = False
												curr_cwnd += (temp.seq - expect_seq) / pkt_len
												expect_seq = temp.seq + pkt_len
									else:
										if self.LOSS_EMULATION_STARTED == False:
											continue

								# whether to emulate loss now
								if self.SHOULD_EMULATE_LOSS == True:
									if self.LOSS_EMULATION_CWND_THRESHOLD != 0:
										if curr_cwnd == self.LOSS_EMULATION_CWND_THRESHOLD and self.LOSS_EMULATION_STARTED == False:
											self.LOSS_EMULATION_STARTED = True
											self.LOSS_RTT = self.CURR_RTT
											self.LOSS_SEQ_CHECKER = temp[TCP].seq
											expect_seq = temp[TCP].seq

								if self.LOSS_EMULATION_STARTED == False:
									pkts_received.append([temp[TCP].seq, temp[TCP].ack, pkt_len, data[pkt_offset:len(data)], temp.time])
									self.DATA_TO_ACK_QUEUE.put([temp[TCP].seq + pkt_len, temp.time, self.EMULATED_RTT - wait_time, 0, self.SEQ])
									if temp[TCP].seq + pkt_len > self.ACK:
										self.ACK += pkt_len
								else:
									if self.LOSS_EMULATION_CWND_THRESHOLD != 0:
										if self.CURR_RTT == self.LOSS_RTT:
											continue
										else:
											if self.CURR_RTT > self.LOSS_RTT:
												if self.LOSS_RECOVERY_DONE == False:
													if temp[TCP].seq == self.LOSS_SEQ_CHECKER:
														self.LOSS_RECOVERY_DONE = True
														self.RTT_AT_RETRANS = self.CURR_RTT
														self.DATA_TO_ACK_QUEUE.put([temp.seq+pkt_len, temp.time, self.EMULATED_RTT - wait_time, 0, self.SEQ])
														self.DATA_TO_ACK_QUEUE.put([temp.seq+pkt_len, temp.time, self.EMULATED_RTT - wait_time, 0, self.SEQ])
													else:
														pass
												else:
													...
													self.DATA_TO_ACK_QUEUE.put([temp[TCP].seq + pkt_len, temp.time, self.EMULATED_RTT - wait_time, 0, self.SEQ])
													if self.FRTO == True:
														if temp.seq == seq_before_recover:
															self.FRTO_handled = True
															self.DATA_TO_ACK_QUEUE.put([seq_before_recover + (self.FRTO_NEW+1) * pkt_len, temp.time, self.EMULATED_RTT - wait_time, 0, self.SEQ])
															expect_seq = seq_before_recover + (self.FRTO_NEW +1) * pkt_len
					...
					if no_pkt_received_yet == True:
						if (time.time() - start_time) >= self.EMULATED_RTT:
							self.ACK_QUEUE.put(1)
							no_data_received_last_rtt = True
							break
					else:
						if (temp.time - first_pkt_receive_time) >= self.EMULATED_RTT:
							self.ACK_QUEUE.put(1)
							no_data_received_last_rtt = False
							break
			...
			if loss_flag == 1 and self.TLS_HS_DONE == False:
				...
				print("loss experienced")
				self.LAST_CWND = curr_cwnd

			if curr_cwnd > self.MAX_CWND:
				self.MAX_CWND = curr_cwnd
			a = self.DATA_TO_ACK_QUEUE.qsize()
			b = self.SECONDARY_QUEUE.qsize()
			wait_begin_time  = time.time()
			while (self.DATA_TO_ACK_QUEUE.empty() != True or self.SECONDARY_QUEUE.empty() != True and a+b >0):
				a = self.DATA_TO_ACK_QUEUE.qsize()
				b = self.SECONDARY_QUEUE.qsize()
			time.sleep(0.05)
			self.ACK_QUEUE.get()
			...
			wait_time = wait_end_time - wait_begin_time
			print("waitng for sending ACK for "+str(wait_end_time - wait_begin_time))
			print("counting from time "+str(start_time)+ " to "+ str(end_time))
			print("recevied packets from" +str(first_pkt_receive_time)+" to "+str(temp.time))
			print('Time elapsed: %lf | CURR_RTT: %d | CWND: %d | RTT: %f'%(end_time-start_time, self.CURR_RTT, self.LAST_CWND, self.EMULATED_RTT))
			print('loss_flag:', loss_flag, '| dup_flag:', dup_flag, '| counter:', counter, '| TLS_HS_DONE:', self.TLS_HS_DONE)
			print('queue_len:', self.DATA_TO_ACK_QUEUE.qsize(), self.SECONDARY_QUEUE.qsize())

			counter += 1
			cw = csv.writer(open(self.dir + 'trace.csv', 'a'))

			cw.writerow([curr_cwnd, self.EMULATED_RTT, self.CURR_RTT, self.START_DATA_ACKED, self.END_DATA_ACKED, time.time(), timestamp_arr])
			self.CURR_RTT += 1
			if self.INCREASE_RTT == True:
				if self.LOSS_RECOVERY_DONE == True and self.EMULATED_RTT <2.0:
					self.EMULATED_RTT += 0.02
			...
			if self.RTTS_WITH_NO_DATA_CONSECUTIVE >= 3 and self.LOSS_EMULATION_STARTED == False:
				if self.MAX_CWND >=600:
					self.DATA_TO_ACK_QUEUE.put([0, first_pkt_receive_time, self.EMULATED_RTT, 1, 0])
					self.ACK_PROCESS.terminate()
					raise Exception('maybe no data')
				else:
					self.TEST_FAIL = True


			if self.RTT_AT_RETRANS > 0 and  self.CURR_RTT - self.RTT_AT_RETRANS>= 20:
				self.DATA_TO_ACK_QUEUE.put([0, first_pkt_receive_time, self.EMULATED_RTT, 1, 0])
				self.ACK_PROCESS.terminate()
			...
			if True:
				try:
					...
					print(e)
					print("but if fails at here")
					raise_imp_exception('EXCEPTION: Resp held. Waiting for more data. ' + str(cn), 2)
					pass

		if self.TEST_FAIL == True:
			#删除cw和log文件 delete trace and log file
			os.remove(self.dir + 'trace.csv')
			#exit
			self.ACK_PROCESS.terminate()
			raise Exception('Test failed due to some reason')
	...
	def do_round_trip_without_recv(self, pkt, recv=False):
		return tls_do_round_trip(self, pkt, False)
